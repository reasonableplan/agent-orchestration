...
| Agent | Domain | 기본 모델 | 역할 |
|-------|--------|-----------|------|
| **Director** | orchestration | Opus 4.6 | 사용자 요청을 Epic/Task로 분해, 에이전트에 할당, 코드 리뷰 |
| **Git** | git | Haiku 4.5 | 브랜치 생성, 커밋, PR 관리, 충돌 해결 |
| **Frontend** | frontend | Sonnet 4.6 | React/UI 코드 생성, 컴포넌트 작성 |
| **Backend** | backend | Sonnet 4.6 | API, 서버 로직, DB 스키마 코드 생성 |
| **Docs** | docs | Haiku 4.5 | README, API 문서, 주석 생성 |
...
**에이전트별 모델/프로바이더 분리**: 각 에이전트가 독립적인 LLM 클라이언트를 사용합니다.
기본값은 Director=Opus, Backend/Frontend=Sonnet, Git/Docs=Haiku이며,
`AgentConfig`의 `llm_provider`, `llm_base_url`, `llm_api_key` 필드로 에이전트별로
GPT, Gemini, Ollama 등 OpenAI 호환 API를 개별 지정할 수 있습니다.
